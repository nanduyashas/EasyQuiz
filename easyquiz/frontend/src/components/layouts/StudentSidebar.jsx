...
import logo from "../../assets/images/logo.png"; // Make sure this exists
...
const StudentSidebar = () => {
  ...
  const menuItems = [
    ...
    { label: "Profile", icon: <FaUserCircle />, path: "/StudentProfile" },
  ...
  return (
    <>
      {/*  ☰ Button — only visible on small screens (below navbar) */}
      {isMobile && !isOpen && (
        <button
          onClick={toggleSidebar}
          className="fixed top-[85px] left-4 z-40 bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-2 rounded-full shadow-lg hover:scale-110 transition-all duration-300 focus:outline-none"
        >
          <FaBars size={18} />
        </button>
      )}
